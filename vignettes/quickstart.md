...
```
    ...
    ## 1      illegal 1.1834716
    ## 2      amnesty 1.1716428
    ## 3    illegally 1.1232888
    ## 4   immigrants 1.0070933
    ## 5  immigration 0.9809758
    ## 6 undocumented 0.9008638
    ## 7       broken 0.7631982
...
```
    ...
    ## 924 instances of "immigration" found.
    ## total observations included in regression: 924 
    ## starting permutations 
    ## done with permutations 
    ##   coefficient normed.estimate.orig normed.estimate.deflated
    ## 1     party_R            10.139940                 8.668252
    ## 2    gender_M             6.908761                 5.154713
    ##   normed.estimate.beta.error.null   n n_obs covariate_mean p.value
    ## 1                        1.471687 924   924      0.4740260       0
    ## 2                        1.754048 924   924      0.6709957       0
...
magnitude of these coefficients we employ the squared Euclidean norm. In
small and/or unbalanced samples, this measure is upwardly biased. To
address this issue, we implement a bias correction as detailed in
[Statistical Bias in Distance
Measures](https://github.com/prodriguezsosa/conText/blob/master/vignettes/bias_in_distance_measures.md).

The `conText-class` contains two important attributes:

- `@normed_coefficients`: a table with
  - the uncorrected squared norm of the coefficients (excluding the
    intercept)
  - the debiased squared norm of the coefficients
  - the estimate of the bias term in the the original squared norm
  - if `jackknife = TRUE`, the std. error (of the squared normed
    coefficients) and lower and upper bound of the confidence interval
    (where bounds are determined based on the `confidence_level`;
    default is 0.95)
  - if `permute = TRUE`, the empirical p-value
- `@features`: the set of features used when creating the embeddings.

To speed up the jackknife computation, iterations can be run in parallel
by setting `parallel=TRUE`. For this functionality to work, users must
register a parallel backend prior to calling `conText()`. For example,
...
and 1 to perform the jackknife on a random subsample of the data, where
the subsample size = `jackknife_fraction * n`. While we recommend that
all final estimates are computed using the full sample, this option can
be useful for exploratory analysis.

### Bias correction in earlier versions of package

In earlier versions of the package, the `@normed_coefficients` table
returned the norm (i.e. square root of the squared norm) of the
coefficients. Bias in these estimates was mitigated by using the
jackknife to estimate and subtract the bias term from the norm. However,
some bias remained in small samples and highly imbalanced groups. As
shown in [Green et
...
our new bias correction method is robust to these issues.

Users interested in accessing previous versions of the package for
backward compatibility or replication purposes can install an older
version using the following `devtools` command:
...
```
    ...
    ##    target feature     value
    ## 1      5%  reform 0.6692875
    ## 2     10%  reform 0.6644484
    ## 18    90%  reform 0.4734498
    ## 19    95%  reform 0.4680526
...
```
    ...
    ##    target feature     value
    ## 20     5% enforce 0.4565472
    ## 21    10% enforce 0.4707653
    ## 37    90% enforce 0.6425993
    ## 38    95% enforce 0.6440665
...
```
    ...
    ## INFO  [18:37:36.122] epoch 1, loss 0.2279
    ## INFO  [18:37:37.049] epoch 2, loss 0.0771
    ## INFO  [18:37:37.927] epoch 3, loss 0.0500
    ## INFO  [18:37:38.882] epoch 4, loss 0.0376
    ## INFO  [18:37:39.753] epoch 5, loss 0.0304
    ## INFO  [18:37:40.620] epoch 6, loss 0.0257
    ## INFO  [18:37:41.610] epoch 7, loss 0.0223
    ## INFO  [18:37:42.480] epoch 8, loss 0.0198
    ## INFO  [18:37:43.421] epoch 9, loss 0.0179
    ## INFO  [18:37:44.445] epoch 10, loss 0.0163
